                ...
                db.query(`SELECT * FROM custom_evaluation_table WHERE ID='${req.query.id}'`,function(err,data){
                    if(err){
                        ...
                        if(data.length==0){
                            ...
                            db.query(`SELECT * FROM custom_evaluation_table`,function(err,custom_data){
                                if(err){
                                    console.log(err);
                                    res.status(500).send('database error').end();
                                }else{
                                    res.render('admin/custom.html',{custom_data,mod_data:data[0]});
                                }
                            });
                        }
                    }
            ...
            default :
                db.query(`SELECT * FROM custom_evaluation_table`,function(err,custom_data){
                    if(err){
                        ...
                        res.render('admin/custom.html',{custom_data});
                    }
                });
                break;
        }
